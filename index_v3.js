...
class UniversalSearchBot {
    ...
    async findTargetClinicGoogle() {
        ...
        try {
            ...
            if (!this.googleBrowser || !this.googleBrowser.page) {
            ...
            for (const selector of morePlacesSelectors) {
                try {
                    const button = await this.googleBrowser.$(selector);
                    if (button) {
            ...
            if (!moreButtonClicked) {
                for (const xpath of morePlacesXPaths) {
                    try {
                        const buttons = await this.googleBrowser.$x(xpath);
                        if (buttons.length > 0) {
            ...
            while (hasMorePages) {
                ...
                for (const selector of cardSelectors) {
                    try {
                        const cards = await this.googleBrowser.$$(selector);
                        if (cards.length > 0) {
                            for (let i = 0; i < cards.length; i++) {
                                try {
                                    const card = cards[i];
                                    const cardText = await this.googleBrowser.evaluate(el => el.textContent || '', card);

                                    // Проверяем, похоже ли это на карточку организации
                                    //if (cardText.length > 50) {
                                    // Проверяем, не добавляли ли уже эту карточку
                                    const cardId = await this.googleBrowser.evaluate(el => el.id || '', card);
                ...
                for (let i = 0; i < totalCards; i++) {
                    try {
                        ...
                        try {
                            ...
                            if (titleElement) {
                                title = await this.googleBrowser.evaluate(el => el.textContent?.trim() || '', titleElement);
                            }
                        } catch (e) {
                        ...
                        try {
                            ...
                            if (divs.length >= 3) {
                                ...
                                const divText = await this.googleBrowser.evaluate(el => el.textContent?.trim() || '', possibleAddressDiv);
                                console.log(divText);
    ...
    async goToNextPageGoogle(currentPage) {
        try {
            ...
            for (const selector of navSelectors) {
                try {
                    const element = await this.googleBrowser.$(selector);
                    if (element) {
                    ...
                    if (elements.length > 0) {
                        ...
                        for (const element of elements) {
                            try {
                                ...
                                const href = await this.googleBrowser.evaluate(
                                    el => el.getAttribute('href'), 
                                    element
                                );
                                const ariaLabel = await this.googleBrowser.evaluate(
                                    el => el.getAttribute('aria-label') || '', 
                                    element
                                );
                                const text = await this.googleBrowser.evaluate(
            ...
            if (nextButton) {
                ...
                await this.googleBrowser.scrollIntoView(nextButton);
                // await this.delayByType('small');

                // Получаем текущий URL для отладки
                const currentUrl = this.googleBrowser.url();
                ...
                const newUrl = this.googleBrowser.url();
