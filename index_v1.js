...
class UniversalSearchBot {
    constructor(targetQuery, profileId = 1) {
        ...
        // Целевые данные для Яндекс
        this.yandexTarget = targetQuery.yandexTarget;

        // Целевые данные для Google
    ...
    // Проверка на капчу (улучшенная версия)
    ...
    async handleCaptcha(engine = 'yandex') {
        ...
        try {
            ...
            // ПАУЗА перед действиями с капчей (чтобы не было слишком быстро)
            await this.delayByType('small');
            ...
            // Если не нашли стандартные селекторы, пробуем XPath для Яндекс
            if (!captchaClicked && engine === 'yandex') {
                const xpaths = [
                    '//input[@type="submit" and contains(@class, "CheckboxCaptcha")]',
                ...
                for (const xpath of xpaths) {
                    try {
                        ...
                        if (elements.length > 0) {
                            ...
                            console.log(`✅ Кликнули по капче через XPath: ${xpath}`);
                            captchaClicked = true;
                            break;
                        }
                    } catch (error) {
            ...
            console.log('⏳ Ждем реакцию капчи (5-10 секунд)...');
            await this.delayByType('medium');
            ...
            if (!captchaStillPresent) {
                ...
                if (engine === 'yandex') {
                    ...
                    // Пробуем нажать Space или Enter на элементе
                    try {
                        const checkbox = await this.page.$('.CheckboxCaptcha-Button');
                        if (checkbox) {
                            await checkbox.focus();
                            await this.page.keyboard.press('Space');
                            console.log('✅ Нажали Space на чекбоксе');
                            await this.delayByType('medium');
                        }
                    } catch (error) {
                        // Игнорируем
                ...
                console.log('🚫 Не удалось решить капчу, закрываем браузер...');

                // Закрываем браузер только если капча не ушла
            ...
            // При любой ошибке закрываем браузер и ждем
    ...
    async executeWarmupQuery(query, iteration) {
        ...
        try {
            ...
            if (await this.checkForCaptcha('yandex')) {
                ...
                // Если капча решена, продолжаем выполнение
            }
    ...
    async run() {
        try {
            ...
            console.log(`🔄 Один профиль для Яндекс и Google`);
